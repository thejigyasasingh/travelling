 ...
 *
 * A near-copy of the admin panel's suite, deliberately. The three clients are
 * separate copies of the same shape — see the note at the top of `core/http.ts`
 * for why they are not a shared package — so the *tests* are duplicated too.
 * A shared test would pass while one copy of the client silently diverged.
 *
 ...
 * signs the user out of every device. A portal that loads eight widgets at
 * once, each getting a 401 and each refreshing independently, would sign
...
describe('request', () => {
  ...
  it('drops query parameters that are unset', async () => {
    ...
    await api.get('/properties', { status: undefined, q: '', page: 2 })
...
describe('refresh', () => {
  ...
  it('refreshes once for many concurrent 401s', async () => {
     ...
     * widgets refreshing in parallel would present the same token eight times
     * and revoke the family — signing the host out of every device, on every
     * token expiry.
     */
    let refreshes = 0
